...
//           <h2 className="text-lg font-semibold">MA (table 4)</h2>
//           {/* Content here */}
//         </div>
...
//   useEffect(() => {
//     if (customer?.name) {
//       fetchDoctorSummary(customer.name); // Fetch summary once customer details are loaded
//     }
//   }, [customer]);
...
//   async function fetchDoctorSummary(doctorName: string) {
//     try {
...
//       // Combine summaries into one string for simplicity
//       const combinedSummary = summaries.map((item: any) => `${item.url}: ${item.summary}`).join('\n\n');
...
//         setSummary('No research papers found for this doctor.');
//       } else {
//         // Combine summaries into one string for simplicity
//         const combinedSummary = summaries.map((item: any) => `${item.url}: ${item.summary}`).join('\n\n');
//         setSummary(combinedSummary);
